...
function validate(req, res, callback, options) {
  ...
  var getCASUser = (options.getCASUser || function(xml) {
    return xml.user;
  });

  logger.info('Start validating ticket...');
  logger.info('Using custom getCASUser: ' + !!options.getCASUser);
  ...
  if (ticket) {
    ...
    validateTicket(req, options, function(err, response) {
      ...
      if (response.status === 200) {
        logger.info('Parsing CAS response');
        ...
        parseCasResponse(response.body, logger, function(err, info) {
          ...
          logger.info("CAS Response", info);
          logger.info("CAS User", getCASUser(info));
          
          if (!info || (info && !getCASUser(info))) {
            return callback(function(req, res, next) {
              res.status(401).send({
